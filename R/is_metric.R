...
#' Check if provided data is metric data as expected
#'
#' @param data A data.frame with metrics data.
#'
is_metric <-  function(data) {
  ...
  expected_cols <- c("step", "scope", "metric", "value")
  expected_classes <- c("integer", "character", "character", "numeric")
